...
function getDefaultRecordInfo() {
    return {
        data: {
            path: (0, path_identity_1.createPathRecord)(),
            uuid: (0, path_identity_1.createPathRecord)(),
        },
        version: DependencyManager.version,
    };
}
...
const associatedMap = (0, path_identity_1.createPathRecord)();
/**
...
class DependencyManager {
    ...
    async _restoreCache(path) {
        ...
        ].map((relativePath) => (0, path_1.join)(this.pathRoot, relativePath));
        (0, path_identity_1.assertNoPathIdentityConflicts)(restoredSourcePaths);
        ...
        Object.keys(cacheInfo.data.path).forEach((relativePath) => {
            ...
            cacheInfo.data.path[relativePath].forEach((subPath) => {
                const dependencyPath = (0, path_1.join)(this.pathRoot, subPath);
                if ((0, path_identity_1.findPathAwareIndex)(restoredDependencies, dependencyPath) === -1) {
                    restoredDependencies.push(dependencyPath);
                }
            });
            pathRecord[(0, path_1.join)(this.pathRoot, relativePath)] = restoredDependencies;
        });
        ...
        for (let type in this.dependMap) {
            ...
            for (let path in typeMap) {
                ...
                array.forEach((urlOrPathOrUUID) => {
                    ...
                    if ((0, path_identity_1.findPathAwareIndex)(associatedMap[urlOrPathOrUUID], path) !== -1) {
                        return;
                    }
    ...
    add(type, key, depends) {
        const typeMap = this.dependMap[type] = this.dependMap[type] || (0, path_identity_1.createPathRecord)();
        const array = typeMap[key] = typeMap[key] || [];
        if (!Array.isArray(depends)) {
            depends = [depends];
        }
        depends.forEach((depend) => {
            if (depend && (0, path_identity_1.findPathAwareIndex)(array, depend) !== -1) {
                return;
            }
            // 记录到依赖列表
            array.push(depend);
            ...
            if ((0, path_identity_1.findPathAwareIndex)(associated, key) === -1) {
                associated.push(key);
            }
        });
        this.save();
    ...
    remove(type, key) {
        ...
        this.dependMap[type][key].forEach((str) => {
            ...
            const index = (0, path_identity_1.findPathAwareIndex)(array, key);
            if (index !== -1) {
    ...
    updatePathCase(previousPath, nextPath) {
        for (const type in this.dependMap) {
            const typeMap = this.dependMap[type];
            (0, path_identity_1.replacePathRecordKey)(typeMap, previousPath, nextPath);
            Object.keys(typeMap).forEach((key) => {
                typeMap[key].forEach((value, index) => {
                    if ((0, path_identity_1.isSamePath)(value, previousPath)) {
                        typeMap[key][index] = nextPath;
                    }
                });
            });
        }
        (0, path_identity_1.replacePathRecordKey)(associatedMap, previousPath, nextPath);
        ...
        for (let type in this.dependMap) {
            ...
            for (let key in typeMap) {
                ...
                array.forEach((str) => {
                    ...
                    const index = (0, path_identity_1.findPathAwareIndex)(associatedMap[str], key);
                    if (index !== -1) {
