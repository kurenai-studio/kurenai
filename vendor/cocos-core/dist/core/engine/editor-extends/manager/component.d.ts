...
export default class ComponentManager extends EventEmitter {
    ...
    _caseInsensitivePathMap: Map<string, string[]>;
    _uuidToPath: Map<string, string>;
    _addOriginPathToCaseInsensitivePathMap(lowercasePath: string, originalPaths: string): void;
    /**
     * 添加一个组件的菜单项
     * @param component
     * @param path
    ...
    _removeComponentPath(uuid: any): void;
    ...
    _getUuidFromLowercasePath(path: string): {
        code: number;
        errMsg: string;
        uuid: string;
    };
