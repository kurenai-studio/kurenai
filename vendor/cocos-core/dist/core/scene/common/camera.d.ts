...
export type IPublicCameraService = Pick<ICameraService, 'focus' | 'defaultFocus' | 'rotateCameraToDir' | 'changeProjection' | 'setGridVisible' | 'isGridVisible' | 'setCameraProperty' | 'resetCameraProperty' | 'queryConfig' | 'updateConfig' | 'getCameraFov' | 'zoomUp' | 'zoomDown' | 'zoomReset' | 'alignNodeToSceneView' | 'alignSceneViewToNode' | 'setGridColor' | 'setOriginAxes2D' | 'setOriginAxes3D'> & {
    is2D: boolean;
};
