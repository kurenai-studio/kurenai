...
var ProjectControl = class {
	...
	async initialize(projectPath, template) {
		...
		if (templateRoot) await copyDirectoryContents(templateRoot, target);
			...
			const headlessRoot = configuredHeadlessRoot ? resolve(configuredHeadlessRoot) : bundledHeadlessRoot();
			if (!existsSync(headlessRoot)) throw new Error("KURENAI_HEADLESS_ROOT must point to a headless-cocos checkout (or use the vendored headless/ tree)");
...
function bundledHeadlessRoot() {
	return resolve(dirname(fileURLToPath(import.meta.url)), "../../headless");
}
function resolveHeadlessRoot(config) {
	const configured = config.headlessRoot ?? process.env.KURENAI_HEADLESS_ROOT ?? process.env.HEADLESS_STACK;
	...
	throw new Error("headlessRoot is required (set KURENAI_HEADLESS_ROOT or use the vendored headless/ tree)");
}
//#endregion
//#region src/index.ts
const name = "kurenai";
