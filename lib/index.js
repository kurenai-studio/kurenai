...
*/
const DEFAULT_COCOS_CLI_ROOT = LEGACY_PINK_COCOS_CLI_ROOT;
function looksLikeCocosCli(root) {
...
function resolveCocosCliRoot(configured) {
	...
	const managed = managedCocosCoreRoot(process.env.KURENAI_COCOS_CORE_VERSION || "4.0.0-alpha.33");
...
async function ensurePacks(packIds, options = {}) {
	...
	if (!existsSync(root) && !options.fetch && !options.baseUrl && !process.env.KURENAI_COCOS_PACK_BASE_URL) throw new Error(`cocos-cli not found: ${root}. Install PinK/cocos-cli or set KURENAI_COCOS_CLI_ROOT. For on-demand packs set KURENAI_COCOS_PACK_BASE_URL (see docs/cocos-cli-split.md).`);
	const alreadyPresent = [];
	const missing = [];
	for (const id of required) if (inspectPack(id, root).present) alreadyPresent.push(id);
	else missing.push(id);
	const installed = [];
	const wantFetch = options.fetch === true || Boolean(options.baseUrl) || Boolean(process.env.KURENAI_COCOS_PACK_BASE_URL);
	if (missing.length && wantFetch) {
		const baseUrl = options.baseUrl ?? process.env.KURENAI_COCOS_PACK_BASE_URL;
		if (!baseUrl) throw new Error(`Missing cocos packs ${missing.join(", ")} under ${root}, and no KURENAI_COCOS_PACK_BASE_URL to fetch them.`);
		const version = options.version ?? loadManifest().measuredAgainst?.engineVersion ?? "4.0.0-alpha.33";
		...
		const hints = missing.map((id) => {
			if (id === "core") return `${id}: install cocos-cli core (dist/cli.js) at ${root}`;
			if (id.startsWith("platform:")) return `${id}: run with a full PinK install or fetch on-demand pack (see docs/cocos-cli-split.md)`;
			return `${id}: native/tool pack not present under ${root}`;
		});
		throw new Error(`Missing cocos packs: ${missing.join(", ")}.\n${hints.join("\n")}\nSet KURENAI_COCOS_PACK_BASE_URL to enable on-demand download, or install the full cocos-cli.`);
	}
	return {
		root,
		required,
...
//#endregion
export { DEFAULT_COCOS_CLI_ROOT, KURENAI_COCOS_CORE_VERSION, KURENAI_PROTOCOL_VERSION, LEGACY_PINK_COCOS_CLI_ROOT, PreviewBridge, PreviewController, ProjectControl, ensureCorePack, ensurePacks, formatSelectionContext, injectInspector, inspectPack, isInspectorMessage, listKnownPackIds, managedCocosCoreRoot, packsForPlatform, packsStatus, resolveCocosCliRoot };
